...
Proof.
  intros n. induction n as [|n' IHn'].
  - reflexivity.
  - { assert (H: negb (negb (evenb (S n'))) = evenb (S n')). {rewrite -> negb_involutive.}} *)
...
S n' + (m + p) = m + (S n' + p) => simpl.
S (n' + (m + p)) = m + S (n' + p) => IHn'
S (m + (n' + p)) = m + S (n' + p) => plus_n_Sm *)
Proof.
  intros n m p. induction n as [|n' IHn'].
  - reflexivity.
...
Proof.
  ...
  - rewrite -> mult_n_Sm. rewrite -> mult_n_Sm. rewrite -> mult_n_Sm.
    rewrite -> IHp'.
    assert (H1: m + (n * p' + m * p') = n * p' + (m + m * p')).
    { rewrite -> plus_swap. reflexivity. }
    assert (H2: n + m + (n * p' + m * p') = n + (m + (n * p' + m * p'))).
    { rewrite <- plus_assoc. reflexivity. }
    simpl. rewrite -> H2. rewrite -> H1. rewrite -> plus_assoc. reflexivity.
Qed. *)
Proof.
  intros n m p. induction n as [|n' IHn'].
  - reflexivity.
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  | Z => 0
  | A m' => mult (bin_to_nat m') 2
  | B m' => S ( mult (bin_to_nat m') 2 )
  end.
...
Example bin_nat_bin_1 : nat_to_bin (bin_to_nat (Z)) = Z.
